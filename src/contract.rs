...
const CONTRACT_NAME: &str = "Out-Token-Factory";
...
pub fn execute(
    ...
    match msg {
        ExecuteMsg::Deposit(deposit_type) => match_deposit(deps.as_ref(), _env, info, deposit_type),
        ExecuteMsg::Burn {
            amount,
            token_address,
...
pub fn execute_instantiate_token(
    ...
    env: Env,
    info: MessageInfo,
    mut token_data: token::msg::InstantiateMsg,
) -> Result<Response, ContractError> {
